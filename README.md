...
The script checks for both on startup and prints the exact install command
for your distro if either is missing.
...
Follow the [Installation](#installing-handbrakecli--bc) steps above for your
distro, then open a new terminal (or run `hash -r`) so your shell picks up
the newly installed binary.

### `bc: command not found`
Install it the same way — `sudo apt-get install bc` / `sudo dnf install bc`
/ `sudo pacman -S bc` — then re-run the script.
